...
export function LiveViewer({
  ...
  const shouldFetchEval = engineEnabled || gaugeEnabled;
  const evalDebounceMs = engineEnabled ? 150 : 320;
  const {
    eval: engineEval,
  ...
  const subscribeGateToast = subscribeGateOpen ? (
    <div className="fixed bottom-4 right-4 z-50 pointer-events-none">
      <div className="pointer-events-auto w-64 rounded-2xl border border-white/10 bg-slate-900/90 px-4 py-3 text-sm text-slate-100 shadow-xl backdrop-blur">
        <div className="text-sm font-semibold">Subscribe to switch boards</div>
        <p className="mt-1 text-xs text-slate-400">Unlock multi-board access for this broadcast.</p>
        <div className="mt-3 flex items-center gap-2 text-xs">
          <Link
            href="/subscribe"
            className="rounded-full border border-emerald-400/50 bg-emerald-400/10 px-3 py-1.5 font-semibold text-emerald-200"
          >
            Subscribe
          </Link>
          <button
            type="button"
            onClick={() => setSubscribeGateOpen(false)}
            className="rounded-full border border-white/10 px-3 py-1.5 text-slate-300 transition hover:border-white/40 hover:text-white"
          >
            Not now
          </button>
        </div>
